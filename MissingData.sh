...
##--current working directory, and outputs a table of the data in an R-readable format. 

echo "INFO      | $(date) |          If MrBayes consensus tree file ('.con.tre' extension) present, split file, extract terminal "
echo "INFO      | $(date) |          branch lengths (term_brL), and tabulate branch length data including CIs. "
shopt -s nullglob
if [[ -n $(echo *.con.tre) ]]; then

	echo "INFO      | $(date) |          Found '.con.tre' file. Reading in MrBayes tree from current directory... "
	##--Starting from file output directly from MrBayes, with extension "*.con.tre",
...
mbTreeStatMiner
    ...
    echo "WARNING!  | $(date) |          No '.con.tre' tree file in current working directory... "
    echo "INFO      | $(date) |          Assuming from this point forward that BEAST tree file(s) are available... "
fi
...
##--that FIX issues with echoing shell text containing dollar signs to R:
	...
	MY_EDGE_LENGTH_VAR=$(echo "\$edge.length")	## This one is not used, but is here in case we need it later...
